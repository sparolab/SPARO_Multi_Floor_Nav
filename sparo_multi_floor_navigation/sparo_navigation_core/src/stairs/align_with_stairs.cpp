...
: BT::StatefulActionNode(name, config)
{
  ...
  node_ = std::make_shared<rclcpp::Node>("align_with_stairs_" + std::to_string(instance_count++));
...
BT::NodeStatus AlignWithStairs::onRunning()
{
  ...
  RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                       "[AlignWithStairs] current_yaw=%.3f target_yaw=%.3f diff=%.3f (tol=%.3f)",
                       yaw, target_yaw_, diff, yaw_tolerance_);
